...
void MooncakeKVCacheTransferDefault::register_kv_cache(
    ...
    const KVCacheShape& kv_cache_shape,
    torch::ScalarType dtype) {
  const bool is_spec_draft = main_registered_;
  CHECK(!is_spec_draft || !spec_registered_)
  ...
  if (!is_spec_draft) {
    ...
    has_index_cache_ = has_index_cache;
    buf_cnt_per_layer_ = buf_cnt_per_layer;
    main_registered_ = true;
  } else {
    spec_num_layers_ = num_layers;
    spec_has_v_cache_ = has_v_cache;
    spec_has_index_cache_ = has_index_cache;
    spec_buf_cnt_per_layer_ = buf_cnt_per_layer;
    spec_buf_offset_ = num_layers_ * buf_cnt_per_layer_;
    spec_registered_ = true;
  }
...
std::vector<int64_t> MooncakeKVCacheTransferDefault::get_buf_ids(
  ...
  const int64_t num_layers = is_spec_draft ? spec_num_layers_ : num_layers_;
  const bool has_v_cache = is_spec_draft ? spec_has_v_cache_ : has_v_cache_;
  const bool has_index_cache =
      is_spec_draft ? spec_has_index_cache_ : has_index_cache_;
  const int64_t buf_cnt_per_layer =
      is_spec_draft ? spec_buf_cnt_per_layer_ : buf_cnt_per_layer_;
  const int64_t buf_offset = is_spec_draft ? spec_buf_offset_ : 0;

  if (is_spec_draft) {
    CHECK(spec_registered_) << "Spec draft kv cache is not registered.";
  } else {
    CHECK(main_registered_) << "Main kv cache is not registered.";
  }

  std::vector<int64_t> active_layer_ids;
  if (layer_ids.empty()) {
    active_layer_ids.resize(static_cast<size_t>(num_layers));
  ...
  buf_ids.reserve(active_layer_ids.size() *
                  static_cast<size_t>(buf_cnt_per_layer));
  for (int64_t layer_id : active_layer_ids) {
    CHECK_GE(layer_id, 0) << "layer_id must be non-negative";
    CHECK_LT(layer_id, num_layers) << "layer_id out of range";

    int64_t buf_id = buf_offset + layer_id * buf_cnt_per_layer;
