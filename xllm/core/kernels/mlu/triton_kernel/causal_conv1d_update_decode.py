    ...
    for flat_pid in range(pid, total_blocks, num_jobs):
        ...
        if should_process:
            ...
            if query_start_index != query_end_index:
                ...
                cover_lo = actual_state_len - actual_seqlen  # = S - L, 运行时但与 a 无关
                new_state_rows = tl.empty((state_len, BLOCK_N), dtype=oldState.dtype)
                FULL_LEN_I: tl.constexpr = FULL_LEN  # new_conv_state 总行数, 用于 clamp
                for gi in tl.static_range(state_len):
                    ...
                    # 当 cover_lo < 0 (varlen 短序列 a<L, S-L<0) 时, 非稳态分支
                    # src_row 可能越出 [0, FULL_LEN); 基准靠 tl.where(mask,...) 的读侧
